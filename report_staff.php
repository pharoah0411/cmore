    ...
    <main class="flex-1 ml-72 p-12">
        ...
        <div id="reportContent">
            ...
            <div class="grid gap-6 xl:grid-cols-3 mb-10">
                ...
                <div class="rounded-[2rem] bg-white p-8 border border-slate-100 shadow-xl shadow-slate-200/40">
                    <?php 
                    // Reset the pointer to avoid issues if we need to query, but for aggregate we run a new query
                    $total_revenue = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COALESCE(SUM(TOTAL_AMOUNT),0) AS total FROM SALES")); 
                    ?>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-left border-collapse">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                <?php 
                                mysqli_data_seek($staff_stats, 0); // Ensure pointer is at the start
                                while($staff = mysqli_fetch_assoc($staff_stats)): 
                                ?>
